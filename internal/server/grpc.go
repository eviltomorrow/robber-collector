...
import (
	...
	"github.com/eviltomorrow/robber-collector/pkg/middleware"
	"github.com/eviltomorrow/robber-collector/pkg/model"
	"github.com/eviltomorrow/robber-collector/pkg/pb"
...
func (g *GRPC) PullData(req *wrapperspb.StringValue, resp pb.Collector_PullDataServer) error {
	var (
		offset  int64 = 0
		limit   int64 = 100
		lastID  string
	...
	for {
		data, err := model.SelectMetadataRange(mongodb.DB, offset, limit, Source, req.Value, lastID, timeout)
		if err != nil {
...
func StartupGRPC() error {
	...
	pb.RegisterCollectorServer(server, &GRPC{})
