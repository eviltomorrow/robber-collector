...
var archiveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for begin.Before(end) {
			var start = time.Now()
			count, stock, day, week, err := service.PushMetadataToRepository(cfg.Collect.Source, begin.Format("2006-01-02"))
			if err != nil {
...
func init() {
	...
	archiveCmd.Flags().StringVarP(&cfgPath, "config", "c", "config.toml", "robber-collector's config file")
