...
func Run() {
	var c = cron.New()
	_, err := c.AddFunc(DefaultCronSpec, do)
	if err != nil {
	...
	defer func() {
		if !ok {
			...
			if _, err := client.SendEmail(ctx, &pb.Mail{
				...
				Subject: "[Warnning] Robber-collector Notify",
				Body:    fmt.Sprintf("<h5>Fetch metadata failure, please check in log, date: [%v]</h5>", time.Now().Format("2006-01-02 15:04:05")),
			}); err != nil {
